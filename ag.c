...
int  avaliacao(indvo *ppl){
	...
	for(i = 0 ; i < qtddisc;i++){
		...
		for(k = 0; k < 30;k+=2){
			flag = 0;
			for(l = 0; l < ppl->qtd;l++){
				if((i == ppl->genes_indv[l].disc && strcmp(ppl->genes_indv[l].prof,dsa[i].nome)==0) &&
				  ppl->genes_indv[l].dia_sem == k){
			...
			if(flag){
				for(j= 0; j < ppl->qtd;j++){
					if((i == ppl->genes_indv[j].disc && strcmp(ppl->genes_indv[j].prof,dsa[i].nome)==0)&&
					  ppl->genes_indv[j].dia_sem != k+1){
	...
	for(i = 0; i < qtdprof;i++){
		...
		for(k = 0; k < 30;k+=2){
			...
			flag = 0;
			for(l = 0;l < ppl->qtd; l++){
				if(k == ppl->genes_indv[l].dia_sem && strcmp(ppl->genes_indv[l].prof,nome)==0){
					if(!flag) {flag=1;break;}	
				}
			}
			if(flag){
				for(j = 0; j < ppl->qtd;j++){
					if(strcmp(ppl->genes_indv[j].prof,nome)==0 && ppl->genes_indv[j].dia_sem==k+1){
						soma+=1;	
						break;			
					}		
 ...
 *
 * @param char nome - Recebe uma string contendo o nome
 *
 * Retorna o índice contendo o semestre a ser pesquisado.
 * 
 **/ 
int getSemIndex(char nome[20]){

	if(strcmp("V1",nome)==0)return 5;
	else if(strcmp("M3",nome)==0)return 4;
	else if(strcmp("M5",nome)==0)return 5;
	else if(strcmp("M7",nome)==0)return 6;
	else if(strcmp("N2",nome)==0)return 3;
	else if(strcmp("N4",nome)==0)return 4;
	else if(strcmp("N6",nome)==0)return 2;
	else if(strcmp("N8",nome)==0)return 3;
	else if(strcmp("N10",nome)==0)return 3;
 ...
 * @param int val
 *
 * Retorna a posição inicial para comparação
 * 
 **/ 
int getAux(int val){

	if(val == 0)return 0;
	else if(val == 1)return 7;
	else if(val == 2)return 11;
	else if(val == 3)return 16;
	else if(val==4) return 22;
	else if(val==5)return 25;
	else if(val == 6)return 29;
	else if(val == 7)return 31;
	else return 34;
}
 ...
 *
 * @param int geracao - Número da melhor geração
 * @param int iteracao - Melhor Iteração do torneio
 * @param indvo *ppl - Melhor indivíduo 
 *
 * A função serve para imprimir os detalhes sobre o melhor
 * indivíduo. São imprimidos a quantidade de horários não
 * preferidos por cada professor em relação a cada semestre.
 * É exibida a quantidade de horários consecutivos, alternados
 * e a distribuição dos horários. No final imprime-se a quantidade
 * total.
 * 
 **/ 
void debugFunction(int geracao, int iteracao, int id, indvo *ppl){

	
	int i,j,tam,k,l;
	int flag = 0;
	int soma = 0;
	int *hora = (int *)malloc(sizeof(int)*qtdprof);
	
	char nome[20];
	
		
	printf("Melhor Geracao: %d | Iteracao Nro : %d  | Individuo Nro : %d \n",
	geracao,iteracao,id);

	//soma = 0;
	
	soma = 0;
	printf("\nHorarios nao Preferidos/semestre");
	for(l=0;l<qtdsem;l++){
		for(i = 0; i < qtdprof;i++)
			hora[i] = 0;
		printf("\nSemestre %s \n", sm[l].se);
		for(i = 0; i < getSemIndex(sm[l].se);i++){
			int test = getAux(l);
			strcpy(nome,dsa[test+i].nome);
			
			if(hora[achaProf(pf,nome)] == 0){
				printf("%s:", nome);
				tam = 0;
				for(j = 0; j <ppl->qtd;j++){
					
				
					if(strcmp(ppl->genes_indv[j].sem,sm[l].se)==0 && 
			   		strcmp(ppl->genes_indv[j].prof,nome)==0){
			   		int idx = achaProf(pf,nome);
			   	
			 			for(k = 0; k < pf[idx].num;k++){
							if(pf[idx].horarios[k] == ppl->genes_indv[j].dia_sem){
														
								tam += 1;
								soma+=1;
								printf("%d ", ppl->genes_indv[j].dia_sem);
							}
						}  
				...
				if(tam == 0)printf("Nenhum");
			   	printf("\n");	
			}
			hora[achaProf(pf,nome)] = 1;		
		}
	
	}
	
	printf("\n Penalidade(1x) horarios nao preferidos : %d", soma);
	printf("\n Penalidade(4x) horarios consecutivos : %d", ppl->erro[0]);
	printf("\n Penalidade(2x) horarios alternados(manha e noite) : %d", ppl->erro[2]);
	printf("\n Penalidade(1x) horarios distribuidos (Dias diferentes) : %d", ppl->choques);
	printf("\n+--------------------------------------------------------------+");	
	printf("\n Penalidade total : %d", ppl->qtdpr + ppl->choques);
...
int main(int argc, char *argv[ ] ){
	...
	for(i = 0 ; i < GERACOES;i++){
		...
		for(j = 0; j < TORNEIO;j++){
			...
			if(prob < PROBCRUZAMENTO){
				...
				if(ind1 == id1 || ind2 == id1){count+=1;
					gera = gera + (GERACOES / count) ;
					ite = j;				
				
				}
		
			}
			id = achaMelhor(populacao);
			if((populacao->individuos[id].qtdpr + populacao->individuos[id].choques) < 
				populacao->individuos[id1].qtdpr+populacao->individuos[id1].choques){
				id1 = id;
	...
	debugFunction(gera, ite, id1,&populacao->individuos[id1]);
	
	printf("\n");	
	imprime(&populacao->individuos[id1]);
